...
export class AuthController {
  ...
  private extractIp(req: any, body?: any): string {
    const rawIp =
      ...
      req?.ip ||
      '192.168.1.1 (Mobile App)';
    return typeof rawIp === 'string'
      ? rawIp.split(',')[0].trim()
      : '192.168.1.1 (Mobile App)';
  }
  ...
  async login(@Body() body: any, @Req() req: any) {
    return this.authService.login(
      body.email || body.emailOrPhone || body.phone,
      body.password,
      this.extractIp(req, body),
    );
  }
  ...
  @ApiOperation({
    summary: 'Direct Fingerprint Login & Auto-Account Creation without OTP',
  })
  ...
  async verifyOtp(@Body() body: any, @Req() req: any) {
    return this.authService.verifyOtp(
      body.identifier || body.email || body.emailOrPhone || body.phone,
      body.otp,
      this.extractIp(req, body),
    );
  }
